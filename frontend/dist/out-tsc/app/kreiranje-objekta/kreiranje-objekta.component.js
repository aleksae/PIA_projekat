...
let KreiranjeObjektaComponent = class KreiranjeObjektaComponent {
    constructor(cdRef, http, router, broadcastService) {
        this.cdRef = cdRef;
        this.http = http;
        this.router = router;
        this.broadcastService = broadcastService;
        this.prostorije = [];
        this.pomeranje_u_toku = false;
        this.prozor_s = null;
        this.prozor_v = null;
        this.aktivno_crtanje = false;
    ...
    postavi_select(prostorija) {
        ...
        if (this.izabran_za_postavljanje == prostorija) {
            ...
            this.izabran_za_postavljanje = prostorija;
    }
    /**
    ...
    proveri_preklapanje(event, x1, y1, s1, v1) {
        ...
        if (s1 > this.canvas_sirina || v1 > this.canvas_visina)
            return 0;
        return 3;
    }
    iscrtaj_svg(i) {
        ...
        if (this.izabran_za_postavljanje == null) {
            if (this.selektovana_vrata != null) {
                ...
                if ((((px - 3) < x && (px + 3) > x) ||
                    ((ps - 3) < x && (ps + 3) > x) ||
                    ((py - 3) < y && (py + 3) > y) ||
                    ((pv - 3) < y && (pv + 3) > y)) && (x >= px && x <= ps && y >= py && y <= pv)) {
                    //sve ok
                    this.greska = null;
                    if ((px - 4) < x && (px + 4) > x)
                        this.vrata[this.selektovana_vrata].strana = 1;
                    else if ((ps - 4) < x && (ps + 4) > x)
                        this.vrata[this.selektovana_vrata].strana = 3;
                    else if ((py - 4) < y && (py + 4) > y)
                        this.vrata[this.selektovana_vrata].strana = 4;
                    else
                        this.vrata[this.selektovana_vrata].strana = 2;
                }
                else {
    ...
    dodaj() {
        ...
        this.http.post('http://localhost:4000/objekti/dodaj', objekat).subscribe((resp) => {
            try {
                if (resp['message'] == 'ok') {
...
__decorate([
    ViewChild('skica_objekta')
], KreiranjeObjektaComponent.prototype, "canvas", void 0);
__decorate([
    HostListener('window:resize', ['$event'])
], KreiranjeObjektaComponent.prototype, "onResize", null);
KreiranjeObjektaComponent = __decorate([
    Component({
...
export { KreiranjeObjektaComponent };
//# sourceMappingURL=kreiranje-objekta.component.js.map